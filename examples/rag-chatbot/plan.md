...
**Verify:**
`docker compose up`, then `curl http://localhost:8000/health` returns `{"status": "ok"}`
...
**Context:**
A RAG chatbot API that stores PDFs as text chunks with vector embeddings. Uses SQLAlchemy async with PostgreSQL and pgvector. OpenAI text-embedding-3-small (1536 dimensions). Chunks ~500 tokens with ~50 overlap.
...
**Verify:**
`curl -F "file=@test.pdf" http://localhost:8000/api/v1/documents` returns document ID and chunk count. `curl http://localhost:8000/api/v1/documents` lists it. Delete returns 200, then list shows it gone.
...
**Context:**
A RAG chatbot API with documents stored as embedded chunks in pgvector. The chat endpoint takes a question, finds relevant chunks via vector similarity, and generates an answer using OpenAI chat completion. Answers include source references. No relevant chunks = "no information" response.
...
**Verify:**
Upload a document, then `curl -X POST http://localhost:8000/api/v1/chat -H "Content-Type: application/json" -d '{"message": "..."}'` returns an answer with sources. Unrelated query returns "no information."
...
**Context:**
A RAG chatbot API with document upload, listing, deletion, and chat endpoints. All responses need consistent JSON structure and proper error handling.
...
**Verify:**
`pytest` — all tests pass. `curl -F "file=@test.txt" http://localhost:8000/api/v1/documents` returns 400.
